...
class View(object):
    ...
    def __main_view(self, properties, frequencies, spectrogram):
        ...
        self.__fig, self.__ax = BasicView.basic_view("Radio camera", mosaic=mosaic, width_ratios=[15, 1, 3, 15], height_ratios=[5, 1, 0.1, 2])

        self.__im["spectrogram"] = self.__ax["spectrogram"].imshow(
            X=spectrogram["magnitude"],
            norm=cm.colors.PowerNorm(gamma=self.__config.data["gamma"], vmin=vmin, vmax=vmax),
        ...
        self.__ax["spectrogram"].grid(**BasicView.grid_arguments())
        ...
        self.__ax["t_proj"].set_ylabel(f"Magnitude [{spectrogram["um"]["magnitude"][1]}]")
        self.__ax["t_proj"].set_xlim(self.__ax["spectrogram"].get_xlim()[0:2])
        self.__ax["t_proj"].margins(x=0)
        self.__ax["t_proj"].grid(**BasicView.grid_arguments())
        ...
        self.__gamma_slider.on_changed((lambda event: self.__gamma_slider_changed(event, self.__im["spectrogram"], vmin, vmax)))
        ...
        self.__band_selection_radiobuttons = RadioButtons(
            ...
            labels=list(
                map(lambda x: f"{x} {self.__config.data['bands'][x]}", self.__config.data["bands"].keys())
            ),
        )
        ...
        self.__ax["t_power"].grid(**BasicView.grid_arguments())
        ...
        self.__ax["f_power"].grid(**BasicView.grid_arguments())
