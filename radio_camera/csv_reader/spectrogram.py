...
def spectrogram(file, config):
    ...
    relative_time = (
        relative_tss_zero_start  # relative time interval with respect to start
    )
    absolute_time = absolute_tss  # absolute timestamp
    ...
    um["magnitude"] = tuple(list(set(map(lambda x: extract_quantity_and_unit(x), columns[1:-1])))[0])
